...
## Approach & Efficiency
Progression through both linked lists occurs using a common while loop while two unique "current" position trackers pace through their respective linked lists.  Two temporary variables are used to retain the next_node location as next_node attributes are overwritten.  The time related Big O for this operation is O(n).  For the most part, the memory is Big O(1).  However, if the linked list node's themselves contain a large amount of data, when that data is stored in the temporary value... it could still have a Big O(n) impact?
